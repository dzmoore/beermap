...
public class BeerBarcodeServlet extends HttpServlet {
			...
			while (iter.hasNext()) {
			    ...
			    if (item.isFormField()) {
			        ...
			        if (filePath == null) {
			        	...
			        	out.println("Barcode data decoded as: [" + barCodeData + "]<br />");
			        	try {
							List<MappedBeer> beers = BeerSearchEngine.getInstance().doSearchByUpc(barCodeData);
							if (beers.size() <= 0) {
							}
						} catch (Exception e) {
							e.printStackTrace();
	...
	private void postUpcToInsertServlet(String upc) {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp)
		...
		resp.setContentType("text/html");
		PrintWriter out = resp.getWriter();
		out.println("<html><body><h1>BeerBarcode</h1>");
		out.println(
